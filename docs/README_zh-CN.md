...
在 7,762 个文件的 Python 代码库（Sentry）上基准测试，涵盖 30 个真实工程任务提示词：

| 指标 | 不使用 GrapeRoot | 使用 GrapeRoot | 节省幅度 |
|------|:---------------:|:--------------:|:-------:|
| 每条提示费用 | $0.77 | **$0.44** | **减少 43%** |
| 每轮读取 token 数 | ~307K | **~76K** | **减少 75%** |
| 每个任务平均轮数 | 16.8 | **10.3** | **减少 39%** |
| 质量评分 | 78.6 / 100 | **78.7–79.4 / 100** | 持平或更优 |
| 性价比（每元质量） | 1.0× | **1.75×** | **提升 75%** |

### 按任务类型划分的节省幅度

图谱仅读取每个文件的相关片段，而非整个文件。节省效果在会话中持续累积：在一个 20 轮对话中第 3 轮节省的 token，在后续每一轮中也同样免于缓存重计费。

| 任务类型 | 读取 token 节省 | 成本降低 |
|---------|:--------------:|:-------:|
| 简单查询 / 单文件 | 50–60% | 5–10% |
| 修复 Bug 与调试 | 65–75% | 15–25% |
| 重构（多文件） | 75–80% | 25–35% |
| 大型代码库导航（7k+ 文件） | **80%+** | **最高 47%** |

> 在大型代码库中，每个会话的 token 读取量减少 **68–75%**。质量持平或提升——AI 获得的是正确文件，而非靠猜测。
