...
7,762ファイルのPythonコードベース（Sentry）で30プロンプト・実際のエンジニアリングタスクをベンチマーク：

| 指標 | GrapeRoot なし | GrapeRoot あり | 削減 |
|------|:--------------:|:--------------:|:----:|
| プロンプトあたりのコスト | $0.77 | **$0.44** | **43% 削減** |
| 1ターンあたりの読み取りトークン数 | ~307K | **~76K** | **75% 削減** |
| タスクあたりの平均ターン数 | 16.8 | **10.3** | **39% 削減** |
| 品質（スコア） | 78.6 / 100 | **78.7–79.4 / 100** | 同等以上 |
| コスパ（1ドルあたりの品質） | 1.0× | **1.75×** | **75% 向上** |

### タスクタイプ別の削減効果

グラフは各ファイルの関連する抜粋のみを読み取ります。全体ではありません。削減効果はセッション全体で複利的に積み重なります。20ターンセッションのターン3で節約したトークンは、それ以降の全ターンでのキャッシュ再請求も回避できます。

| タスクタイプ | 読み取りトークン削減 | コスト削減 |
|------------|:-----------------:|:--------:|
| 単純な検索 / 単一ファイル | 50–60% | 5–10% |
| バグ修正とデバッグ | 65–75% | 15–25% |
| リファクタリング（複数ファイル） | 75–80% | 25–35% |
| 大規模コードベースのナビゲーション（7k+ ファイル） | **80%+** | **最大 47%** |

> 大規模コードベースでは、1セッションあたりのトークン読み取り量が **68–75% 減少** します。品質は同等以上を維持——AIが推測ではなく正しいファイルを受け取るためです。
